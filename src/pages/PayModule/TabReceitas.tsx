...
export function TabReceitas() {
  ...
  useEffect(() => {
    const entries = payStorage.getEntries().filter(e => e.tipo === 'receita')
  ...
  const handleExport = () => {
    const entries = payStorage.getEntries().filter(e => e.tipo === 'receita' && new Date(e.dataVenc).getFullYear() === ano)
