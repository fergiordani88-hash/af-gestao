...
import { payStorage, type PayCompany } from '../../services/payStorage'
import { Card } from '../../components/ui/Card'

const EMPTY: PayCompany = {
...
export function TabEmpresa() {
  const [form, setForm]   = useState<PayCompany>(EMPTY)
  const [saved, setSaved] = useState(false)
  const [dirty, setDirty] = useState(false)

  useEffect(() => {
    const c = payStorage.getCompany()
    if (c) setForm(c)
  }, [])

  const set = (k: keyof PayCompany, v: string) => { setForm(f => ({ ...f, [k]: v })); setDirty(true); setSaved(false) }

  const handleSave = () => {
    payStorage.saveCompany(form)
