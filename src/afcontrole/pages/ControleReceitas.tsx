...
export function ControleReceitas() {
  ...
  useEffect(() => {
    const entries = controleStorage.getEntries().filter(e => e.tipo === 'receita')
    ...
    const m = Array.from({ length: 12 }, (_, i) => {
      ...
      return {
        mes: MESES[i],
        recebido:  mv.filter(e => e.status === 'pago').reduce((s, e) => s + e.valor, 0),
        previsto:  mv.filter(e => e.status !== 'pago').reduce((s, e) => s + e.valor, 0),
      }
    })
    setMensal(m)
  ...
  return (
    <ControleLayout title="Análise de Receitas" subtitle="Evolução mensal e distribuição por fonte de receita">
      <div className="space-y-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="font-bold text-gray-900">Análise de Receitas</h2>
            <p className="text-xs text-gray-500 mt-0.5">Evolução mensal e distribuição por fonte de receita</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="col-span-2 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-1"><TrendingUp size={16} className="text-emerald-600" /><p className="text-xs text-gray-500">Total de Receitas {ano}</p></div>
          ...
          <Card className="p-5">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={mensal}>
                ...
                <Bar dataKey="previsto" name="Previsto" fill="#A5D6A7" radius={[3,3,0,0]} stackId="a" />
              </BarChart>
            </ResponsiveContainer>
          </Card>
        )}
        ...
        {porCat.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <Card className="p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={porCat} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`}>
