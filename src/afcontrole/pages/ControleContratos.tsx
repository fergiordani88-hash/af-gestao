...
import { fetchSelicRate, generateInstallments, countInstallments, calcPMT } from '../../services/contractUtils'
...
export function ControleContratos() {
  ...
  const nParcelas = (() => {
    ...
    const pseudo = { ...form, id: '__tmp__', createdAt: '' } as ControleContract
  ...
  const handleSave = async () => {
    ...
    const finalForm: Omit<ControleContract, 'id' | 'createdAt'> = {
      ...form,
    ...
    if (editItem?.id) {
      controleStorage.updateContract(editItem.id, finalForm)
      contractId = editItem.id
      controleStorage.deleteEntriesByContract(contractId)
      setEditItem(null)
    } else {
      const c = controleStorage.addContract(finalForm)
      contractId = c.id
    }

    // Gera e salva parcelas no fluxo de caixa do AF Controle
    const fullContract = { ...finalForm, id: contractId, createdAt: new Date().toISOString() } as ControleContract
    const installments = generateInstallments(fullContract, selicRate)
    installments.forEach(inst => controleStorage.addEntry({
      tipo: inst.tipo,
      categoria: inst.categoria,
      descricao: inst.descricao,
      valor: inst.valor,
      dataVenc: inst.dataVenc,
      status: inst.status,
      recorrente: inst.recorrente,
      periodicidade: inst.periodicidade,
      contrato: inst.contrato,
      obs: inst.obs,
    }))
  ...
  const handleDelete = (id: string) => {
    ...
    controleStorage.deleteEntriesByContract(id)
    controleStorage.deleteContract(id)
    load()
  }
  ...
  return (
    <ControleLayout title="Cronograma de Contratos" subtitle="Contratos recorrentes — parcelas geradas automaticamente no fluxo de caixa">
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="font-bold text-gray-900">Cronograma de Contratos</h2>
            <p className="text-xs text-gray-500 mt-0.5">Ao salvar, todas as parcelas são inseridas automaticamente no fluxo de caixa</p>
          </div>
          ...
          <Card className="p-5 border-2 border-yellow-200">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-2 mb-3">
                <TrendingUp size={14} className="text-[#C9A258]" />
              ...
              {form.tipoTaxa && (
                <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                    ...
                    <p className="font-bold text-[#C9A258] text-lg">{fmtBRL(pmtPreview)}</p>
                    <p className="text-[10px] text-gray-400 mt-0.5">
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={handleSave} disabled={saving}
                className="bg-[#C9A258] hover:bg-[#b8913f] text-white rounded-xl px-4 py-2 text-sm font-semibold flex items-center gap-2 disabled:opacity-60">
